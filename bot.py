        ...
        with open(env_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line and "=" in line and not line.startswith("#"):
                    k, v = line.split("=", 1)
                    if k.strip() == "TG_TOKEN" and not token:
                        token = v.strip()
                    elif k.strip() == "NVIDIA_API_KEY" and not nvidia_key:
                        nvidia_key = v.strip()
                        
    if not token or not nvidia_key:
        print("ERROR: Please provide TG_TOKEN and NVIDIA_API_KEY via environment variables or .env file!", file=sys.stderr)
        sys.exit(1)
